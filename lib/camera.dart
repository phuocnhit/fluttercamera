...
typedef onLatestImageAvailableNative = Function(dynamic image);
...
class CameraController extends ValueNotifier<CameraValue> {
  ...
  Future<void> detectorAruco(onLatestImageAvailableNative onAvailable) async {
    if (!value.isInitialized || _isDisposed) {
      throw CameraException(
        'Uninitialized CameraController',
        'startImageStream was called on uninitialized CameraController.',
    ...
    try {
      await _channel.invokeMethod<void>('detectorAruco');
      value = value.copyWith(isStreamingImages: true);
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
